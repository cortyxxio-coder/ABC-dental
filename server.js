...
→ The customer-facing text in THIS message must only say something like "let me check if that time is available" — NEVER say "confirmed", "booked", "ചെയ്തിരിക്കുന്നു", or "ആയിട്ടുണ്ട്" in this message. Availability has not been checked yet at this point, so nothing is confirmed yet.
→ Once you receive the availability result, in your NEXT message ask explicitly: "Shall I confirm/book this appointment?" (in the customer's language). Only after they reply affirmatively to THIS specific question should you proceed to booking.

## CRITICAL RULE — NEVER FALSELY CONFIRM A BOOKING
- You must NEVER tell the customer their appointment is "confirmed", "booked", "ചെയ്തിരിക്കുന്നു", or similar UNLESS you are outputting a [ACTION:BOOK_APPOINTMENT|...] tag in that exact same response.
- Saying the words "confirmed" or "booked" to the customer without including the ACTION:BOOK_APPOINTMENT tag in the same message is STRICTLY FORBIDDEN. The booking only actually happens when the ACTION tag fires — never say it succeeded before that.
- A generic closing question like "anything else I can help you with?" or "മറ്റെന്തെങ്കിലും സഹായം വേണോ?" is NOT the same as asking "shall I confirm your booking?" — never treat a reply to the closing question (like "no" or "ok") as a booking confirmation.
- The ONLY message that should ever be followed by treating "yes/ok/ys/confirm" as a booking confirmation is a message where YOU explicitly asked "shall I confirm/book this appointment?" in that exact turn. If you did not just ask that specific question, do not interpret the next reply as a booking confirmation.

After you have asked "shall I confirm this appointment?" and the customer replies with any clear affirmative — including but not limited to "yes", "ys", "yeah", "ok", "okay", "sure", "confirm", "ശരി", "അതെ", "OK ആണ്" or similar, even with typos or short forms — output:
[ACTION:BOOK_APPOINTMENT|name:PATIENT_NAME|phone:PHONE|date:YYYY-MM-DD|time:HH:MM|treatment:TREATMENT]
Do not require an exact word match.
...
async function checkAvailability(data) {
  try {
    ...
    if (!text) {
      console.error('Check availability: empty response from n8n');
...
async function bookAppointment(data) {
  try {
    ...
    const text = await res.text();
    if (!text) {
      console.error('Booking: empty response from n8n');
      return { success: true }; // assume success since n8n Create Event usually has no explicit body
    }
    return JSON.parse(text);
  } catch (err) {
...
async function cancelAppointment(data) {
  try {
    ...
    const text = await res.text();
    if (!text) {
      console.error('Cancel: empty response from n8n');
      return { success: false };
    }
    return JSON.parse(text);
  } catch (err) {
...
app.post('/chat', async (req, res) => {
  try {
    ...
    // SAFETY NET: never let a false "confirmed/booked" message reach the patient
    // unless we actually fired a BOOK_APPOINTMENT action this turn.
    const claimsConfirmed = /confirm(ed)?|booked|ചെയ്തിരിക്കുന്നു|ബുക്ക്|കൺഫേം/i.test(cleanReply);
    if (claimsConfirmed && (!action || action.type !== 'BOOK_APPOINTMENT')) {
      finalReply = "Sorry, I need one more confirmation before I can finalize this — could you please confirm again: shall I book this appointment for you? (Yes/No)";
    }
